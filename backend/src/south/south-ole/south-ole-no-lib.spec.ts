...
import SouthOLE from './south-ole';
...
describe('SouthODBC without ODBC Library', () => {
  ...
  beforeEach(() => {
    ...
    south = new SouthOLE(configuration, items, addValues, addFile, encryptionService, repositoryService, logger, 'baseFolder');
  });

  it('should throw error if library not loaded', async () => {
    await expect(south.testOleConnection()).rejects.toThrow(new Error('odbc library not loaded'));

    const startTime = '2020-01-01T00:00:00.000Z';
    const endTime = '2022-01-01T00:00:00.000Z';
    await expect(south.queryOleData({} as SouthConnectorItemDTO, startTime, endTime)).rejects.toThrow(
